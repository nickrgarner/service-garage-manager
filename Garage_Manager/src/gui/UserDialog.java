...
public class UserDialog extends JDialog {
	...
	/**
	 * Constructor, sets up the dialog window for Editing a vehicle
	 * 
	 * @param license License of the vehicle passed in from GUI
	 * @param owner   Owner of the vehicle passed in from GUI
	 * @param hybrid  Int value of 1 for hybrid, 0 for regular car
	 */
	public UserDialog(String license, String owner, int hybrid) {
		super((java.awt.Frame) null, TITLE_BAR_EDIT, true);
		setUpGeometryForEdit(license, owner);
		if (hybrid == 1) {
	...
	/**
	 * Private method -- sets layouts, adds panels and widgets for Edit
	 * 
	 * @param license License plate of the Vehicle to edit
	 * @param owner   Owner name of the Vehicle to edit
	 */
	private void setUpGeometryForEdit(String license, String owner) {
		// The bottom panel will have Submit, Cancel buttons
		pnlBtn.setLayout(new FlowLayout());
		pnlBtn.add(btnSubmit);
		pnlBtn.add(btnCancel);
	...
	/**
	 * Private method. Creates the grid to lay out the labels and widgets for Edit
	 * 
	 * @param license License plate of the Vehicle to edit
	 * @param owner   Owner name of the Vehicle to edit
	 */
	private void setInfoForEdit(String license, String owner) {
		// Initialize the layout grid constraints
		pnlInfo.setLayout(new GridBagLayout());
		gbC.weightx = 0.5;
		gbC.gridx = 0;
		...
		for (int k = 0; k < 3; k++) {
			...
			gbC.gridx = 1;
			if (k == 0) {
				compInfo[k] = new JTextField(owner, 20);
				pnlInfo.add(compInfo[k], gbC);
				compInfo[k].setEnabled(false);
			} else if (k == 1) {
				compInfo[k] = new JTextField(license, 20);
				pnlInfo.add(compInfo[k], gbC);
				compInfo[k].setEnabled(false);
			} else {
				compInfo[k] = cboTier;
